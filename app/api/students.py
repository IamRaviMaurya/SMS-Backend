...
# Every route here requires an authenticated user whose tenant context is active.
router = APIRouter(prefix="/students", tags=["Students"], dependencies=[Depends(TenantAuthGuard)])


@router.post("/register", response_model=StudentResponse, status_code=status.HTTP_201_CREATED,
             dependencies=[Depends(AdminGuard)])
def register_student(student_in: StudentCreate, db: Session = Depends(get_db)):
    ...
    except student_service.StudentLimitReached as e:
        raise HTTPException(status_code=403, detail=str(e))
    except student_service.DuplicateGrNumber as e:
...
def get_students_stats(db: Session = Depends(get_db)):
    total = student_service._scoped_query(db).count()
    active = student_service._scoped_query(db).filter(Student.status == "Active").count()
    return {
...
@router.put("/{student_id}", response_model=StudentResponse, dependencies=[Depends(AdminGuard)])
